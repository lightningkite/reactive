...
@JvmInline
@OptIn(InternalReadableApi::class)
value class ReadableState<out T>(val raw: T) {
    ...
    companion object {
        ...
        @Suppress("UNCHECKED_CAST")
        fun <T> exception(exception: Exception) = (if(exception is CancellationException) notReady else ReadableState<Any?>(InternalReadableThrownException(exception))) as ReadableState<T>
        @Suppress("UNCHECKED_CAST")
...
@InternalReadableApi data class InternalReadableWrapper<T>(val other: T)
@InternalReadableApi data class InternalReadableThrownException(val exception: Exception)
@InternalReadableApi object InternalReadableNotReady
